...
PM_START_TEXT = """
Merhaba! Ben [𝐑𝐞𝐠𝐢𝐬𝐭𝐚| ꜱᴏʜʙᴇᴛ 🇹🇷](https://t.me/+uZGX59QUrQIyZmI0) için yapılmış bir Bot'um. Sadece Regista'ya ait Gruplarda kullanılabilirim.
"""

buttons = [
    [
        InlineKeyboardButton(
            text="➕️ 𝐑𝐞𝐠𝐢𝐬𝐭𝐚| ꜱᴏʜʙᴇᴛ 🇹🇷 ➕️", url="https://t.me/+uZGX59QUrQIyZmI0"),
    ],
    [
        InlineKeyboardButton(text="📣 Kanal 📣", url="https://t.me/RegistaFootball"),
        InlineKeyboardButton(text="🃏 Sahibim 🃏", url="tg://openmessage?user_id=2140479641"),
    ],
    [
        InlineKeyboardButton(
...
HELP_STRINGS = """
**Komutlara sadece adminler bakabilir!**
"""


DONATE_STRING = """**Bağış yapmak için [sahibime](tg://openmessage?user_id=2140479641) özelden yazın.**"""
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
            ...
            if args[0].lower() == "help":
                ...
                send_help(
                    ...
                    InlineKeyboardMarkup(
                        [[InlineKeyboardButton(text="⬅️ GERİ", callback_data="help_back")]]
                    ),
                )
        ...
        update.effective_message.reply_text(
            "Ben zaten uyanığım!\n<b>Şu zamandan beri uyumadım:</b> <code>{}</code>".format(
                uptime
            ),
...
def error_handler(update, context):
    ...
    LOGGER.error(msg="Bir güncellemeyi işlerken istisna:", exc_info=context.error)
...
def groupmenter_about_callback(update: Update, context: CallbackContext):
    ...
    if query.data == "groupmenter":
        query.message.edit_text(
            text=""" 🤖 Ben Regista için yapılmış bir Bot'um.
...
❍ Gelişmiş bir sel önleme sistemim var.
...
❍ Bir not tutma sistemim, kara listelerim ve hatta belirli anahtar kelimeler için önceden belirlenmiş yanıtlarım var.""",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="GERİ", callback_data="menter_back")
                 ]
                ]
            ),
        )
...
def Source_about_callback(update: Update, context: CallbackContext):
    ...
    if query.data == "source_":
        query.message.edit_text(
            text=""" Kaynak Kodu'm sahibimdedir. Bence onu bu konuda rahatsız etmeyin. :D""",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=False,
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="GERİ DÖN", callback_data="source_back")
                 ]
                ]
            ),
        )
...
def get_help(update: Update, context: CallbackContext):
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            ...
            return
        update.effective_message.reply_text(
            "Olası komutların listesini almak için PM ile bana ulaşın.",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
...
def send_settings(chat_id, user_id, user=False):
    if user:
        ...
        if CHAT_SETTINGS:
            ...
            dispatcher.bot.send_message(
                user_id,
                "Görünüşe göre herhangi bir sohbet ayarı yok :'(\nMevcut ayarlarını bulmak için "
                "bunu yöneticisi olduğunuz bir grup sohbetinde gönderin!",
                parse_mode=ParseMode.MARKDOWN,
            )
...
def settings_button(update: Update, context: CallbackContext):
    ...
    try:
        if mod_match:
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text(
                "Merhaba! {} için epeyce ayar var - devam edin ve "
                "ilgilendiğiniz şeyi seçin.".format(chat.title),
                reply_markup=InlineKeyboardMarkup(
                    paginate_modules(
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text(
                text="Merhaba! {} için epeyce ayar var - devam edin ve "
                "ilgilendiğiniz şeyi seçin.".format(escape_markdown(chat.title)),
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup(
                    paginate_modules(0, CHAT_SETTINGS, "stngs", chat=chat_id)
                ),
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 1414146649 and DONATION_LINK:
            update.effective_message.reply_text(
                "Ayrıca şu anda beni burada yöneten kişiye de bağışta "
                "[bulunabilirsiniz]({})".format(DONATION_LINK),
                parse_mode=ParseMode.MARKDOWN,
            )
        ...
        try:
            ...
            update.effective_message.reply_text(
                "Ayrıca şu anda benim sahibime de bağışta bulunabilirsiniz."
            )
        except Unauthorized:
            update.effective_message.reply_text(
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendMessage(f"@{SUPPORT_CHAT}", "Kodlarım başarıyla çalıştı!")
        except Unauthorized:
            LOGGER.warning(
                "Bot Regista'ya mesaj gönderemiyor, git ve kontrol et!"
            )
